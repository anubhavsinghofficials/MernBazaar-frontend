...
type dialogueEditReviewPropsType = {
...
function DialogueEditReview(props:dialogueEditReviewPropsType) {

    const { onConfirm, defaultRating, defaultMessage } = props
    const [ rating, setRating ] = useState<number|null>(defaultRating ? defaultRating : null)
    const [ message, setMessage ] = useState<string>(defaultMessage ? defaultMessage : '')
    const { showEditReview, toggleShowEditReview } = modalStore()
    const modalRef = useRef<HTMLDivElement>(null)

    useEffect(()=>{
        setTimeout(() => {
            if (modalRef.current && showEditReview) {
    ...
    useEffect(()=>{
        if (modalRef.current) {
            setMessage(defaultMessage ? defaultMessage : '')
            setRating(defaultRating ? defaultRating : null)
        }
    },[modalRef.current])
...
export default DialogueEditReview